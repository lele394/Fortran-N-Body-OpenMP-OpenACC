program sim
    ...
    ! Main parameters ========
    integer, parameter :: n_stars = 10000 !SE
    integer, parameter :: number_of_steps = 1000 !SE
    ! integer :: save_modulo = 50 !save state every X
    integer :: display_modulo = 10 ! display remaining steps every X
    ...
    logical :: do_V = .False.
    logical :: do_E = .False.
    logical :: save_last_V = .False. ! Allows to load simulation from a previous state 
    ...
    ! Initialization of the accelerations ==============================================
        ...
        do j = 1, n_stars
            ...
            ! Newton
            f_vec = f_vec + (G * mass / (sqrt(sum(dp**2) + epsilon**2)**3)) * dp ! compute the interaction force between the objects
    ...
    do n = 1, number_of_steps
        ...
        !$acc update host(positions) ! kept for fairness
